...
class TCell : public Cell {
 ...
 public:
  TCell() {}
  explicit TCell(const Real3& position, real diameter, size_t color, int t)
      : Base(position), color_(color) {
    activation_histo_ = TH2I("", "", 30, 0, 49, 30, 0, t - 1);
    this->SetDiameter(diameter);
  }

  void IncreaseActivationIntensity(real val) {
    if (val > 0) {
  ...
  void SetInitialActivationIntensity(real mean, real sigma) {
    mean_ = mean;
    sigma_ = sigma;
    auto* r = Simulation::GetActive()->GetRandom();
