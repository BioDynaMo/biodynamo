...
void Client::SetTimeout(duration_ms_t timeout) {
...
void
Client::Send (const std::string& identity, zmqpp::message& msg) {
    ...
    if (verbose) {
        std::cout << "I: send request to '" << identity << "' identity: " << msg << std::endl;
    }
...
bool
Client::Recv (std::string *command_out, std::string *identity_out, zmqpp::message& msg) {
    if ( !sock->receive(msg) ) {
        return false;
    }
