...
class CommandHeader {
 public:
  ...
  static std::unique_ptr<T> FromString(const std::string * str) {
    TBufferFile buf (TBufferFile::EMode::kRead, str->size(), ((void *)str->c_str()));

    // find specialized type
    return std::unique_ptr<T>(
        (T*)buf.ReadObjectAny(T::Class())
    );
  }
...
#define optional_arg(class_, type, name, default_value) \
  public: class_& name(type name) { name##_ = name; return *this; } \
          type name##_ = default_value

class ClientCommandHeader : public CommandHeader {
 public:
  ClientCommandHeader() {};
  ClientCommandHeader(ClientProtocolCmd cmd, CommunicatorId sender, CommunicatorId receiver) :
  ...
  std::unique_ptr<std::string> ToString() {
    ...
    return std::make_unique<std::string>(std::string(buf.Buffer(), buf.Length()));
  }
...
class WorkerCommandHeader : public CommandHeader {
 public:
  WorkerCommandHeader() { };
  WorkerCommandHeader(WorkerProtocolCmd cmd, CommunicatorId sender, CommunicatorId receiver) :
    CommandHeader(sender, receiver), cmd_(cmd) { }
