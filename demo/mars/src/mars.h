...
namespace bdm {
  namespace astrophysics {
    ...
    inline int Simulate(int argc, const char** argv) {
      ...
      phobos->SetPosition({mars->GetPosition()[0] + 9376, mars->GetPosition()[1], mars->GetPosition()[2]});  // Position relative to Mars
      Real3 v_phobos = mars->GetVelocity() + RotateVector({0, 2.138, 0}, 0, 'x');  // Phobos velocity relative to Mars
      ...
      deimos->SetPosition({mars->GetPosition()[0] + 23459, mars->GetPosition()[1], mars->GetPosition()[2]});  // Position relative to Mars
      Real3 v_deimos = mars->GetVelocity() + RotateVector({0, 1.351, 0}, 0, 'x');  // Deimos velocity relative to Mars
