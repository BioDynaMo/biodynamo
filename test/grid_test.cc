...
TEST(GridTest, UpdateGrid) {
  ...
  std::vector<size_t> expected_41 = {20, 21, 22, 24, 25, 26, 28, 29, 30, 36, 37, 38, 40, 43, 44, 45, 51, 52, 53, 55, 56, 57, 59, 60, 61};
  ...
  if (expected_0 != neighbors[0]) { FAIL(); }
  if (expected_3 != neighbors[3]) { FAIL(); }
  if (expected_41 != neighbors[41]) { FAIL(); }
  if (expected_61 != neighbors[61]) { FAIL(); }
}
