...
//
//   for (uint64_t i = 0; i < 10; i++) {
//     EXPECT_EQ(reference.BreitWigner(i, i + 2), random->BreitWigner(i, i +
//     2));
//   }
//
...
//   gRandom->SetSeed(42);
//   auto distrng =
//       random->GetUserDefinedDistRng1D(function, {static_cast<real_t>(1.1),
//       static_cast<real_t>(1.2)}, min, max);
...
// }
//
// // This test is neither a proper death test nor a proper function test. We
// run
// // this test, to see if it is possible to initialize the UserDefinedDistRng1D
// // in a parallel region because the generator is based on ROOT's TF1 class
// which
// // seems to have trouble when the constructor is called in parallel. Before
// the
...
// // in a parallel region because the generator is based on ROOT's TF2 class
// which
// // seems to have trouble when the constructor is called in parallel. Before
// the
// // fix provided in PR #208, this test would fail roughly 2/3 times.
...
// // in a parallel region because the generator is based on ROOT's TF3 class
// which
// // seems to have trouble when the constructor is called in parallel. Before
// the
// // fix provided in PR #208, this test would fail roughly 2/3 times.
