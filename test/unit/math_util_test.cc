...
TEST(MathUtilTest, Add) {
  std::array<float, 3> a = {0.5, 0.7, 1.2};
  std::array<float, 3> b = {0.6, 1.5, 2.1};
...
TEST(MathUtilTest, Subtract) {
  std::array<float, 3> a = {0.6, 1.5, 2.1};
  std::array<float, 3> b = {0.5, 0.7, 0.8};
...
TEST(MathUtilTest, Dot) {
  std::array<float, 3> a = {0.5, 0.7, 0.8};
  std::array<float, 3> b = {0.6, 1.5, 2.1};
...
TEST(MathUtilTest, Norm) {
  ...
  EXPECT_NEAR(4.115823125451335, result, abs_error<float>::value);
}
...
TEST(MathUtilTest, Normalize) {
  ...
  EXPECT_NEAR(0.2672612419124244187, result[0], abs_error<float>::value);
  EXPECT_NEAR(0.5345224838248488374, result[1], abs_error<float>::value);
  EXPECT_NEAR(0.8017837257372732561, result[2], abs_error<float>::value);
}

TEST(MathUtilTest, L2Distance) {
  std::array<float, 3> vector1 = {0, 0, 0};
  std::array<float, 3> vector2 = {1, 2, 3};
  auto result1 = Math::GetL2Distance(vector1, vector2);
  auto result2 = Math::GetL2Distance(vector2, vector1);

  EXPECT_NEAR(3.7416573867739413855, result1, abs_error<float>::value);
  EXPECT_NEAR(3.7416573867739413855, result2, abs_error<float>::value);
}
...
TEST(MathUtilTest, CrossProduct) {
  ...
  std::array<float, 3> b = {5.8, 7.3, 11.87};
...
TEST(MathUtilTest, RotAroundAxis) {
  std::array<float, 3> axis = {1.0, 1.0, 0.0};
  std::array<float, 3> vector = {4, 5, 6};
  float theta = Math::kPi;
...
TEST(MathUtilTest, AngleRadian) {
  ...
  std::array<float, 3> b = {9, 8, 7};

  float result = Math::AngleRadian(a, b);
  EXPECT_NEAR(0.489306575615854, result, abs_error<float>::value);
}
