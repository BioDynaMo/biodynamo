...
#include "unit/separate_binary/sim_object_util_test.h"
...
// Therefore SimObject is extended in two stages: first by MyCellExt and
// then by NeuronExt

TEST(SimObjectUtilTest, ContainerFunctionality) {
...
TEST(SimObjectUtilTest, DefaultConstructor) {
...
TEST(SimObjectUtilTest, NewEmptySoa) {
...
TEST(SimObjectUtilTest, NonDefaultConstructor) {
...
TEST(SimObjectUtilTest, Soa_push_back_AndSubscriptOperator) {
  std::vector<Neurite> neurites;
  neurites.push_back(Neurite(2));
  neurites.push_back(Neurite(3));
...
TEST(SimObjectUtilTest, Soa_clear) {
  SoaNeuron neurons;
...
TEST(SimObjectUtilTest, Soa_reserve) {
  SoaNeuron neurons;
  neurons.reserve(10);
...
TEST(SimObjectUtilTest, Soa_AssignmentOperator) {
  std::vector<Neurite> neurites;
  neurites.push_back(Neurite(2));
  neurites.push_back(Neurite(3));
...
TEST(SimObjectUtilTest, Soa_IO) { RunSoaIOTest(); }
...
TEST(SimObjectUtilTest, IsSoType) {
  Neuron neuron;
...
TEST(SimObjectUtilTest, ReinterpretCast) {
  MyCell cell;
