...
class SpaceNodeDebug : public SpaceNode<T> {
 public:
  std::shared_ptr<Tetrahedron<T> > searchInitialInsertionTetrahedron(const std::shared_ptr<Tetrahedron<T> >& start,
                                                                     const std::array<double, 3>& coordinate) {
    logCall(start, coordinate);
    auto ret = SpaceNode < T > ::searchInitialInsertionTetrahedron(start, coordinate);
    logReturn(ret);
    return ret;
  }

  SpaceNodeDebug(const std::array<double, 3>& position, const std::shared_ptr<T> content)
  ...
  SpaceNode<T>* getNewInstance(const std::array<double, 3>& position, const std::shared_ptr<T>& user_object) {
  ...
  std::array<std::shared_ptr<T>, 4> getVerticesOfTheTetrahedronContaining(const std::array<double, 3>& position,
...
//    logReturn(ret);
  ...
  void addAdjacentTetrahedron(const std::shared_ptr<Tetrahedron<T> >& tetrahedron) {
    logCall(tetrahedron);
  ...
  void setListenerList(const std::list<std::shared_ptr<SpatialOrganizationNodeMovementListener<T> > >& listeners) {
    logCall(listeners);
...
//  }
  ...
  bool removeTetrahedronDuringCleanUp(std::shared_ptr<Tetrahedron<T> >& tetrahedron_to_remove,
