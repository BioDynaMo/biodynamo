...
brew install \
  $(cat $BDM_PROJECT_DIR/util/installation/osx/common/package_list_required) || true
...
if [ $1 == "all" ]; then
    ...
    brew install \
      $(cat $BDM_PROJECT_DIR/util/installation/osx/common/package_list_extra) || true
fi

# Test installation of brew formulae such that the user gets feedback if 
# his/her brew installation did not work as expected.
brew_required=0
for package in $(cat $BDM_PROJECT_DIR/util/installation/osx/common/package_list_required)
do
...
if [ $1 == "all" ]; then
  ...
  brew_extra=0
  for package in $(cat $BDM_PROJECT_DIR/util/installation/osx/common/package_list_extra)
  do
    brew list $package &> /dev/null
    if [ $? -eq 1 ]; then
      brew_extra=1
