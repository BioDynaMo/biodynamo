...
class CatalystAdaptor {
 ...
 private:
  ...
  struct ProcessSimObjectFunctor : public Functor<void, SimObject*> {
    ProcessSimObjectFunctor(CatalystAdaptor* ca,
                            const vtkNew<vtkCPDataDescription>& data_description) : ca_(ca), data_description_(data_description) {}

    void operator()(SimObject* so) {
      ca_->ProcessSimObject(so, data_description_);
    }

    CatalystAdaptor* ca_;
    const vtkNew<vtkCPDataDescription>& data_description_;
  };
  ...
  void BuildSimObjectsVTKStructures(
    ...
    ProcessSimObjectFunctor functor(this, data_description);
