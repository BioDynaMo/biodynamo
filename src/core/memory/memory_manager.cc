...
NumaPoolAllocator::NumaPoolAllocator(uint64_t size, int nid) : size_(size), nid_(nid) {
...
void NumaPoolAllocator::CreateFreeList(void* block, uint64_t mem_block_size, Node** head, Node** tail) {
...
PoolAllocator::PoolAllocator(std::size_t size) : size_(size), tinfo_(ThreadInfo::GetInstance()) {
