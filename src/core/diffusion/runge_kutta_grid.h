...
class RungeKuttaGrid : public DiffusionGrid {
 public:
  RungeKuttaGrid() {}
  RungeKuttaGrid(int substance_id, std::string substance_name, real dc,
                 int resolution = 11)
  ...
  void DiffuseWithClosedEdge(real dt) override;
  void DiffuseWithOpenEdge(real dt) override;

 private:
  /// Buffers for Runge Kutta
  ParallelResizeVector<real> r1_ = {};
