...
class GeneRegulation : public Behavior {
  ...
  void Run(Agent* agent) override {
    ...
    if (param->numerical_ode_solver == Param::NumericalODESolver::kEuler) {
      ...
      for (uint64_t i = 0; i < first_derivatives_.size(); i++) {
        real slope = first_derivatives_[i](absolute_time, concentrations_[i]);
        concentrations_[i] += slope * timestep;
      }
    } else if (param->numerical_ode_solver == Param::NumericalODESolver::kRK4) {
      // Runge-Kutta 4
      for (uint64_t i = 0; i < first_derivatives_.size(); i++) {
        real interval_midpoint = absolute_time + timestep / 2.0;
        real interval_endpoint = absolute_time + timestep;

        real k1 = first_derivatives_[i](absolute_time, concentrations_[i]);
        real k2 = first_derivatives_[i](
            interval_midpoint, concentrations_[i] + timestep * k1 / 2.0);
        real k3 = first_derivatives_[i](
            interval_midpoint, concentrations_[i] + timestep * k2 / 2.0);
        real k4 = first_derivatives_[i](interval_endpoint,
 ...
 private:
  ...
  std::vector<real> concentrations_ = {};
  ...
  std::vector<std::function<real(real, real)>> first_derivatives_ = {};
};
