...
void ParallelExecutionManager::Log(string s) {
  Log::Info("ParallelExecutionManager", "[M]:   ", s);
}
...
void ParallelExecutionManager::WriteTimingsToFile() {
  std::ofstream myfile;
...
void ParallelExecutionManager::RecordTiming(int worker, TimingAggregator *agg) {
  timings_[worker] = *agg;
}

// Register all workers' availability
void ParallelExecutionManager::WaitForAllWorkers() {
...
void ParallelExecutionManager::KillAllWorkers() {
  ForAllWorkers([&](int worker) {
    {
      Timing t_mpi("MPI_CALL", &ta_);
...
void ParallelExecutionManager::GetTimingsFromWorkers() {
  ForAllWorkers([&](int worker) {
    int size;
    MPI_Status status;
    {
...
int ParallelExecutionManager::Start() {
  {
    ...
    // deterimine the algorithm type: e.g. ParameterSweep, Differential
    // Evolution, Particle Swarm Optimization
    OptimizationParam *opt_params =
        default_params_->GetModuleParam<OptimizationParam>();
...
int ParallelExecutionManager::GetFirstAvailableWorker() {
...
// Changes the status
void ParallelExecutionManager::ChangeStatusWorker(int worker, Status s) {
...
void ParallelExecutionManager::ForAllWorkers(
