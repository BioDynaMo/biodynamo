...
inline __host__ __device__ double3 normalize(double3 v) { return v / norm(v); }

__device__ double squared_euclidian_distance(double* positions, uint32_t idx,
                                             uint32_t nidx) {
...
__device__ int3 get_box_coordinates(double3 pos, int32_t* grid_dimensions,
                                    uint32_t box_length) {
  int3 box_coords;
...
__device__ int3 get_box_coordinates_2(uint32_t box_idx,
                                      uint32_t* num_boxes_axis_) {
  int3 box_coord;
...
__device__ uint32_t get_box_id(double3 pos, uint32_t* num_boxes_axis,
                               int32_t* grid_dimensions, uint32_t box_length) {
  int3 box_coords = get_box_coordinates(pos, grid_dimensions, box_length);
  return get_box_id_2(box_coords, num_boxes_axis);
}

__device__ void compute_force(double* positions, double* diameters,
  ...
  // printf("Force between cell (%u) [%f, %f, %f] & cell (%u) [%f, %f, %f] = %f,
  // %f, %f\n", idx, positions[3*idx + 0], positions[3*idx + 1], positions[3*idx
  // + 2], nidx, positions[3*nidx + 0], positions[3*nidx + 1], positions[3*nidx
  // + 2], module * comp1, module * comp2, module * comp3);
}

__device__ void default_force(double* positions, double* diameters,
                              uint32_t idx, uint32_t start, uint16_t length,
                              uint32_t* successors, double* squared_radius,
                              double3* result) {
  uint32_t nidx = start;
  for (uint16_t nb = 0; nb < length; nb++) {
    if (nidx != idx) {
      if (squared_euclidian_distance(positions, idx, nidx) <
          squared_radius[0]) {
...
__global__ void collide(double* positions, double* diameters,
                        double* tractor_force, double* adherence,
                        uint32_t* box_id, double* mass, double* timestep,
                        double* max_displacement, double* squared_radius,
                        uint32_t* num_objects, uint32_t* starts,
                        uint16_t* lengths, uint64_t* timestamps,
                        uint64_t* current_timestamp, uint32_t* successors,
                        uint32_t* box_length, uint32_t* num_boxes_axis,
                        int32_t* grid_dimensions, double* result) {
  ...
  if (tidx < num_objects[0]) {
    ...
    for (int z = -1; z <= 1; z++) {
      for (int y = -1; y <= 1; y++) {
        for (int x = -1; x <= 1; x++) {
          uint32_t bidx =
              get_box_id_2(box_coords + make_int3(x, y, z), num_boxes_axis);
          if (timestamps[bidx] == current_timestamp[0] && lengths[bidx] != 0) {
            default_force(positions, diameters, tidx, starts[bidx],
                          lengths[bidx], successors, squared_radius,
                          &collision_force);
          }
        }
      }
    ...
    if (norm(collision_force) > adherence[tidx]) {
      ...
      // collision_force.y, collision_force.z);
...
void bdm::DisplacementOpCudaKernel::LaunchDisplacementKernel(
  ...
  // printf("[LaunchDisplacementKernel] positions[0] = %f  |  positions[1] =
  ...
  collide<<<gridSize, blockSize>>>(
      d_positions_, d_diameters_, d_tractor_force_, d_adherence_, d_box_id_,
      d_mass_, d_timestep_, d_max_displacement_, d_squared_radius_,
      d_num_objects_, d_starts_, d_lengths_, d_timestamps_,
