...
public:
    ...
    static bool
    point_compare_x(const pair<point, T> x, const pair<point, T> y) {
        return (x.first.x < y.first.x);
    }

    static bool
    point_compare_y(const pair<point, T> x, const pair<point, T> y) {
        return (x.first.y < y.first.y);
    }

    static bool
    point_compare_z(const pair<point, T> x, const pair<point, T> y) {
...
void kd_tree_node<T>::split_on_single_x_median() {
    ...
    for (int i = 0; i < 2; i++) {
        children[i] = new kd_tree_node<T>(
                bound(p[i], e[i]), max_depth - 1,
                max_amount_of_objects, split_);
    }
...
point kd_tree_node<T>::get_sah_split_point() {
    ...
    for (int i = 0; i < N; i++) {
        if (i == 0) {
            ...
            rightside_count[i] = std::accumulate(
                    std::begin(objects_count),
                    std::end(objects_count),
                    0, std::plus<int>()) - objects_count[i];
        } else {
    ...
    sah = ct + ci * ((k_part_surface(1, axis)
                      * leftside_count[0] + k_part_surface(31, axis)) /
                     surface);
    split_point.set(bnd.flb.x + ((bnd.nrt.x - bnd.flb.x) * 1 / N),
                    bnd.flb.y, bnd.flb.z);
    for (int i = 1; i < N - 1; i++) {
        temp = ct + ci * ((k_part_surface(i + 1, axis)
                           * leftside_count[0] +
                           k_part_surface(N - i - 1, axis))
                          / surface);
        if (temp < sah) {
            sah = temp;
            split_point.set(bnd.flb.x + ((bnd.nrt.x - bnd.flb.x) * (i + 1) / N),
                            bnd.flb.y, bnd.flb.z);
            this->axis = 0;
        }
    }
    ...
    for (int i = 0; i < N; i++) {
        if (i == 0) {
            leftside_count[i] = objects_count[i];
            rightside_count[i] = std::accumulate(std::begin(objects_count),
                                                 std::end(objects_count),
                                                 0, std::plus<int>())
                                 - objects_count[i];
        } else {
    ...
    sah = ct + ci * ((k_part_surface(1, axis)
                      * leftside_count[0] + k_part_surface(31, axis))
                     / surface);
    split_point.set(bnd.flb.x, bnd.flb.y + ((bnd.nrt.y - bnd.flb.y) * 1 / N),
                    bnd.flb.z);
    for (int i = 1; i < N - 1; i++) {
        temp = ct + ci * ((k_part_surface(i + 1, axis)
                           * leftside_count[0]
                           + k_part_surface(N - i - 1, axis)) / surface);
        if (temp < sah) {
            sah = temp;
    ...
    sah = ct + ci * ((k_part_surface(1, axis)
                      * leftside_count[0] + k_part_surface(31, axis))
                     / surface);
    split_point.set(bnd.flb.x, bnd.flb.y, bnd.flb.z + ((bnd.nrt.z - bnd.flb.z)
                                                       * 1 / N));
    for (int i = 1; i < N - 1; i++) {
        temp = ct + ci * ((k_part_surface(i + 1, axis)
                           * leftside_count[0]
...
double kd_tree_node<T>::k_part_surface(int k, int axis) {
    if (axis == 0) {
        return this->surface - k / N * (this->bnd.height() * this->bnd.length())
               - k / N * (this->bnd.width() * this->bnd.length());
    } else if (axis == 1) {
        return this->surface - k / N * (this->bnd.height() * this->bnd.width())
               - k / N * (this->bnd.width() * this->bnd.length());
    } else if (axis == 2) {
        return this->surface - (k / N * (this->bnd.height() * this->bnd.width())
                                + k / N *
                                  (this->bnd.height() * this->bnd.length()));
    } else return -1;
}
