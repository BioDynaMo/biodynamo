...
vector<pair<pair<point, T>, pair<point, T> > > *spatial_tree_node<T>::get_neighbors_with_points(double distance) {
    vector<pair<pair<point, T>, pair<point, T> > > *
            result = new vector<pair<pair<point, T>, pair<point, T> > >();
 ...
 */
template <typename T>
pair<pair<point, T>, pair<point, T> > make_neighbor_pair(
        point p1, T o1, point p2, T o2) {
...
template <typename T>
pair<pair<point, T>, pair<point, T> > make_neighbor_pair(
        pair<point, T> p1, pair<point, T> p2) {
 ...
 * @param result - container, where we save retrieved data (all neighbor pairs with its points)
 */
template <typename T>
    ...
    if (A->is_leaf() && B->is_leaf()) {
        ...
        for (int i = 0; i < a_objs->size(); i++)
            for (int j = is_same ? (i + 1) : 0; j < b_objs->size(); j++)
                if ((a_objs->at(i).first).sqdist(
                        b_objs->at(j).first) <= distance)
                    result->push_back(make_neighbor_pair(
                            a_objs->at(i), b_objs->at(j)));
    } else {
        ...
        if (A->is_leaf()) {
            ...
            for (int i = 0; i < B->get_children_size(); i++)
                if (A->get_bound().distance(
                        b_nodes[i]->get_bound()) <= distance)
                    get_neighbors_(A, b_nodes[i], distance, result);
        } else if (B->is_leaf()) {
            a_nodes = A->get_children();
            for (int i = 0; i < A->get_children_size(); i++)
                if (a_nodes[i]->get_bound().distance(
                        B->get_bound()) <= distance)
            ...
            for (int i = 0; i < a_size; i++)
                for (int j = is_same ? (i) : 0; j < b_size; j++)
                    if (a_nodes[i]->get_bound().distance(
                            b_nodes[j]->get_bound()) <= distance)
                        get_neighbors_(a_nodes[i],
                                       b_nodes[j],
                                       distance,
                                       result);
        }
 ...
 * @param B - node of the tree
 * @param distance - finding neighbors within that distance
 * @param result - container, where we save retrieved data (all neighbor pairs without its points)
 */
template <typename T>
void spatial_tree_node<T>::get_neighbors_(
        spatial_tree_node<T> * A,
            ...
            for (int j = is_same ? (i + 1) : 0; j < b_objs->size(); j++)
                if ((a_objs->at(i).first).sqdist(
                    ...
                    result->push_back(make_pair(
                            a_objs->at(i).second, b_objs->at(j).second));
    } else {
        spatial_tree_node<T> ** a_nodes, ** b_nodes;
        int a_size = 0, b_size = 0;
        if (A->is_leaf()) {
