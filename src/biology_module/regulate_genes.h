...
/// through variable d_e_solve_method_choosed_ from structure Param.
...
struct RegulateGenes : public BaseBiologyModule {
  ...
  RegulateGenes(BmEvent event) : BaseBiologyModule(event) {}
  ...
  /// Method Calculate() takes the current simulation time and previos
  /// concentrations
  /// of proteins and calculates new values for proteins by variable functions_
  std::vector<double> const Calculate(double time,
                                      std::vector<double>& previos_value) {
    std::vector<double> updated_value;
    updated_value.reserve(functions_.size());
    for (unsigned int i = 0; i < functions_.size(); i++) {
      updated_value.push_back(functions_[i](time, previos_value[i]));
    }
