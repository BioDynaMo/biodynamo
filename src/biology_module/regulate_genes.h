...
struct RegulateGenes : public BaseBiologyModule {
  ...
  void Run(T* cell) {
    ...
    if (Param::numerical_ode_solver_ == Param::NumericalODESolver::kEuler) {
      ...
      for (uint64_t i = 0; i < first_derivatives_.size(); i++) {
        float slope = first_derivatives_[i](absolute_time, concentrations_[i]);
      ...
      // Runge-Kutta 4
      for (uint64_t i = 0; i < first_derivatives_.size(); i++) {
        float interval_midpoint = absolute_time + timestep / 2.0;
        float interval_endpoint = absolute_time + timestep;

        float k1 = first_derivatives_[i](absolute_time, concentrations_[i]);
        float k2 = first_derivatives_[i](
            interval_midpoint, concentrations_[i] + timestep * k1 / 2.0);
        float k3 = first_derivatives_[i](
            interval_midpoint, concentrations_[i] + timestep * k2 / 2.0);
        float k4 = first_derivatives_[i](interval_endpoint,
 ...
 private:
  ...
  std::vector<std::function<float(float, float)>> first_derivatives_ = {};
