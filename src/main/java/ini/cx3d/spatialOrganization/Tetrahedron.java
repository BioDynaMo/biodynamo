		...
		if (!isInfinite()) {
			...
			for (int i = 0; i < 6; i++)
				if (adjacentEdges[i] != null)
					adjacentEdges[i].addTetrahedron(this);
	...
	public static <T> ini.cx3d.spatialOrganization.interfaces.Tetrahedron[] remove2FlatTetrahedra_java(
		...
		for (int i = 0; (i < 4); i++) {
			...
			for (int j = 0; (j < 4) && (possible); j++)
				possible = !Objects.equals(triangleListA[i], triangleListB[j]);
			if (possible) {
