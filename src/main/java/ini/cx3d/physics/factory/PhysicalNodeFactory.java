...
public class PhysicalNodeFactory {
    ...
    private static final boolean NATIVE = simulation.useNativePhysicalNode;
    public static final boolean DEBUG = simulation.debugPhysicalNode;

    static {
        ini.cx3d.swig.simulation.PhysicalNode.setECM(ECMFacade.getInstance());
    }

    public static PhysicalNode create() {
        if (NATIVE) {
            PhysicalNode node =  ini.cx3d.swig.simulation.PhysicalNode.create();
            ini.cx3d.swig.simulation.PhysicalNode.registerJavaObject(node);
            return node;
        } else if(!DEBUG) {
    ...
    public static double[] getBarycentricCoordinates(double[] Q, double[] P1, double[] P2, double[] P3, double[] P4){
        if (NATIVE) {
            return ini.cx3d.swig.simulation.PhysicalNode.getBarycentricCoordinates(Q, P1, P2, P3, P4);
        } else if(!DEBUG) {
    ...
    public static double[] getBarycentricCoordinates(double[] Q, Object[] vertices){
        if (NATIVE) {
            return ini.cx3d.swig.simulation.PhysicalNode.getBarycentricCoordinates(Q, vertices);
        } else if(!DEBUG) {
