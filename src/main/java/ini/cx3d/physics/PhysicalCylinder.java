...
public class PhysicalCylinder extends physics.PhysicalCylinderBase {//ini.cx3d.swig.physics.PhysicalCylinder{//ini.cx3d.physics.PhysicalObject {
	...
	public boolean isRelative(ini.cx3d.physics.interfaces.PhysicalObject po) {
		try
		{
			//getRwLock().readLock().lock();
			// mother-daughter
			if( (Objects.equals(po, mother))||(Objects.equals(po, daughterLeft))||(Objects.equals(po, daughterLeft)) ) //fixme looks like bug
				return true;
			// sister-sister
	...
	public void removeDaugther(ini.cx3d.physics.interfaces.PhysicalObject daughterToRemove) {
		...
		if(Objects.equals(daughterToRemove, daughterRight)){
			//getRwLock().writeLock().lock();
			daughterRight = null;
			//getRwLock().writeLock().unlock();
			return;
		...
		if(Objects.equals(daughterToRemove, daughterLeft)){
			//getRwLock().writeLock().lock();
	...
	public void updateRelative(ini.cx3d.physics.interfaces.PhysicalObject oldRelative, ini.cx3d.physics.interfaces.PhysicalObject newRelative) {
		...
		if(Objects.equals(oldRelative, daughterLeft)){
			setDaughterLeft((PhysicalCylinder) newRelative);
			return;
		}
		if(Objects.equals(oldRelative, daughterRight)){
	...
	public double[] forceTransmittedFromDaugtherToMother(ini.cx3d.physics.interfaces.PhysicalObject motherWhoAsks) {
		...
		{
			//getRwLock().readLock().lock();
			if(!Objects.equals(motherWhoAsks, mother)){
	...
	private NeuriteElement insertProximalCylinder(double distalPortion){
		...
		try {
			newSON = getSoNode().getNewInstance(newProximalCylinderSpatialNodeLocation, newProximalCylinder);
...
//			double rresting = this.getActualLength()+motherCyl.getActualLength();
		...
		//		double s = 1;
		//		for (PhysicalNode neighbor : getSoNode().getNeighbors()) {
		//		// of course, only if it is an instance of PhysicalObject
		//		if(neighbor instanceof PhysicalCylinder){
