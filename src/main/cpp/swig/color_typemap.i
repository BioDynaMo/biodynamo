...
%define %color(MODULE)
  ...
  %pragma(java) modulecode=%{
    static java.awt.Color convertColor(long cPtr, boolean cMemoryOwn) {
      long color = new ini.cx3d.swig.MODULE.Color(cPtr, cMemoryOwn).getValue();
      return new java.awt.Color((int) color, true);
    }

    static ini.cx3d.swig.MODULE.Color convertColor(java.awt.Color color) {
      return new ini.cx3d.swig.MODULE.Color(color.getRGB());
    }
  %}
  ...
  %typemap(javadirectorout) cx3d::Color "Color.getCPtr( MODULE.convertColor($javacall))"
