...
std::array<double, 3> PhysicalSphere::transformCoordinatesLocalToPolar(const std::array<double, 3>& pos) const {
  return {
    ...
    std::atan2( std::sqrt( pos[0] * pos[0] + pos[1] * pos[1]), pos[2]),
    std::atan2(pos[1], pos[0])
  };
}

std::array<double, 3> PhysicalSphere::transformCoordinatesPolarToLocal(const std::array<double, 3>& pos) const {
  return {
    pos[0] * std::cos(pos[2])* std::sin(pos[1]),
    pos[0] * std::sin(pos[2])* std::sin(pos[1]),
    pos[0] * std::cos(pos[1])
  };
}
...
std::array<double, 3> PhysicalSphere::transformCoordinatesPolarToGlobal(const std::array<double, 2>& pos) const {
  ...
  double localX = pos[0] * std::cos(pos[2]) * std::sin(pos[1]);
  double localY = pos[0] * std::sin(pos[2]) * std::sin(pos[1]);
  double localZ = pos[0] * std::cos(pos[1]);
  return {
...
std::array<double, 3> PhysicalSphere::transformCoordinatesGlobalToPolar(const std::array<double, 3>& pos) const {
  ...
  return {
    std::sqrt(local_cartesian[0] * local_cartesian[0] + local_cartesian[1] * local_cartesian[1] + local_cartesian[2] * local_cartesian[2]),
    std::atan2( std::sqrt(local_cartesian[0] * local_cartesian[0] + local_cartesian[1] * local_cartesian[1]), local_cartesian[2]),
